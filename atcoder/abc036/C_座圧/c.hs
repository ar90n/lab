...
doit xs = doit' (-1) ((-1,-1):xs) where
    doit' :: Int -> [(Int,Int)] -> [(Int,Int)]
    doit' n (e0@(p0,_):e1@(p1,i1):es) = (n',i1) : doit' n' es' where
        n' = if p0 == p1 then n else n+1
        es' = e1:es
    doit' _ _ = []
...
main = do
    ...
    ps <- replicateM n $ readInt1 <$> BS.getLine
    let res = map fst . sortOn (\(_,a) -> a) . doit . sortOn (\(a,_) -> a) $ zip ps [1..]
