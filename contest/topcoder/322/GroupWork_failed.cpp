...
long long bestGroup(vector<int> p, vector<int> s)
{
    ...
    for( int i = 0; i < n; i++ )
    {
        unsigned long long pro1 = (unsigned long long )num * skill;
        unsigned long long pro2 = (unsigned long long )( num + elem[ i ].second ) * elem[ i ].first;
