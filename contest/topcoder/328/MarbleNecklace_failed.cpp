...
string normalize(string necklace)
{
	...
	int right = ( min_index + 1 ) % n;
	int left = ( ( min_index - 1 ) + n ) % n;
	if( necklace[left] < necklace[right] )
	{
		rotate( necklace.begin(),necklace.begin() + min_index + 1,necklace.end() );
		reverse( necklace.begin(),necklace.end());
	}
	else
	{
		rotate( necklace.begin(),necklace.begin() + min_index,necklace.end() );
	}
