...
int minNumber(vector<int> pleasantness, int variety)
{
    ...
    for( int i = 0; i < n; i++ )
    {
        pp.push_back( make_pair( pleasantness[ i ], i ) );
    }
    //sort( pp.begin(), pp.end() );

    int min_prob = pleasantness.size();
    for( int i = 0; i < n ; i++ )
    {
        for( int j = i + 1; j < n; j++ )
        {
            int diff = abs( pp[i].first - pp[j].first );
            if( variety <= diff )
            {
                int first_prob = min( pp[i].second, pp[j].second );
                int second_prob = max( pp[i].second, pp[j].second );
                int first_num = ( first_prob + 1 ) / 2 + 1;
                int second_num = ( ( second_prob - first_prob ) + 1 ) / 2;
                min_prob = min( min_prob, first_num + second_num );
            }
        }
    }
    return min_prob;
