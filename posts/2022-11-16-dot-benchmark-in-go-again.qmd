...
$mkdir -p /tmp/dot_benchmark_in_go_again
%cd /tmp/dot_benchmark_in_go_again
$rm -f go.mod 
$go mod init dot_benchmark_in_go_again
$go get gonum.org/v1/gonum/...@latest
$go get github.com/semi-technologies/weaviate/adapters/repos/db/vector/hnsw/distancer/asm
```
...
for _, f := range []func(a, b []float32) float32 {
    CalcDot,
    CalcDotUnroll2,
    CalcDotUnroll4,
    CalcDotUnroll8,
    CalcDotByGonumBlas,
    CalcDotByWeaviateAsm,
} {
    if f([]float32{1, 2, 3, 4, 5, 6, 7, 8}, []float32{1, 2, 3, 4, 5, 6, 7, 8}) != 204 {
        panic("error")
    }
}
```
