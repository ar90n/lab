...
with BuildPart() as all_body:
    Box(51.4 + 24.0 + 6 * 2, 17.7 + 2.5, 9.0)
    with Locations((26, -1.0, 1.0)):
    ...
    with Locations(all_body.faces().filter_by(Axis.Z).sort_by(Axis.Z).first):
        with Locations(((51.4 + 24.0 + 5.5) / 2, -(17.7 / 2.0 - 1.8))):
            CounterBoreHole(2.1 / 2.0, 4.2 / 2.0, 1.5, 8.5)
        with Locations((-(51.4 + 24.0 + 6.0) / 2, (17.7 / 2.0 - 1.8))):
            CounterBoreHole(2.1 / 2.0, 4.2 / 2.0, 1.5, 8.5)
show(all_body)
# %%
with BuildPart() as bottom_body:
    ...
    with BuildSketch(Plane.XY.offset(1)):
        with Locations(((51.4 + 24.0 + 5.5) / 2, (17.7 / 2.0 - 1.8))):
            RegularPolygon(radius=5.3 / 2.0, side_count=6, rotation=45)
        with Locations((-(51.4 + 24.0 + 6.0) / 2, -(17.7 / 2.0 - 1.8))):
            RegularPolygon(radius=5.3 / 2.0, side_count=6, rotation=45)
