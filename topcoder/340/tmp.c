...
int checkCourse( course &c,  int tv, int pv, int month )
{
    return ( ( c.tv - 1 ) <= tv ) && (( c.pv - 1 ) <= pv ) && (month <= c.exp );
}
vector<int> getOrder(vector<int> theoreticalValue, vector<int> practicalValue, vector<int> expire, int skillBound)
{
    ...
    for( int i = 0; i < n ; i++ )
    {
        course tmp = { theoreticalValue[ i ], practicalValue[ i ], expire[ i ] };
        c.push_back( tmp );
    }
    ...
    while( 0 < que.size() )
    {
        course current = que.head(); que.pop();
    }
//    doit( theoreticalValue, practicalValue, expire, skillBound, mask, t, p, 0 );

    vector<int>res;
    return res;
}

void doit( vector< int > tv, vector<int> pv, vector<int> exp, int sb, long long mask, int t, int p, int m )
{
    if( ( sb <= t ) && ( sb <= p ) )
    {
        cout << mask << endl;
    ...
    for( int i = 0;i < tv.size(); i++ )
    {
        if( ( ( tv[i] - 1 ) <=  t ) &&
            ...
            ( ( mask & ( 1 << i ) ) == 0 )  )
        {
            doit( tv, pv, exp, sb, mask | ( 1 << i ), max( tv[i], t ), max( pv[i], p ), m  + 1 );
        }
    }
